...
OPTIONS_HELP_TEXT = {
    ...
    "source": "Source from which the metadata will be extracted. Available Sources: ['WebnovelDotCom', 'NovelUpdates']",
...
def source_callback(value: str) -> MetadataSource:
    ...
    if value not in AVAILIABLE_SOURCES:
        raise typer.BadParameter(typer.style(f"Available sources are: ['WebnovelDotCom', 'NovelUpdates']. Choose one of them!", fg=typer.colors.RED, bold=True))
...
def request_stalkers_api(
    root: Annotated[str, typer.Option('--root','-r', help=OPTIONS_HELP_TEXT["root"], prompt="Root Folder")] = None,
    novel_path: Annotated[Path, typer.Option('--novel-path','-n', help=OPTIONS_HELP_TEXT["novel_path"], prompt="Novel Path")] = None,
    with_image: Annotated[Optional[bool], typer.Option('--with-image/--without-image', help=OPTIONS_HELP_TEXT["with_image"])] = False
):
    send_request(root_path=root, novel_path=novel_path, with_image=with_image)
